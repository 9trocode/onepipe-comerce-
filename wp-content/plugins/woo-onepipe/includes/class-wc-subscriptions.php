...
class Tbz_WC_Gateway_onepipe_Subscription extends Tbz_WC_onepipe_Gateway {
	...
	*/
	...
	/**
	 * Don't transfer onepipe fee/ID meta to renewal orders.
	 *
	...
	public function scheduled_subscription_payment( $amount_to_charge, $renewal_order ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$renewal_order->update_status( 'failed', sprintf( 'onepipe Transaction Failed (%s)', $response->get_error_message() ) );
	...
	public function process_subscription_payment( $order = '', $amount = 0 ) {
		...
		$auth_code = get_post_meta( $order_id, '_onepipe_token', true );
		...
		if ( $auth_code ) {
	        ...
	        if ( ! is_wp_error( $request ) && 200 == wp_remote_retrieve_response_code( $request ) ) {
				...
				if ( 'success' == $onepipe_response->data->status ) {
					...
					$message = sprintf( 'Payment via onepipe successful (Transaction Reference: %s)', $onepipe_ref );
					...
					$gateway_response = 'onepipe payment failed.';

					if( isset( $onepipe_response->data->gateway_response ) && ! empty ( $onepipe_response->data->gateway_response ) ) {
						$gateway_response = 'onepipe payment failed. Reason: ' . $onepipe_response->data->gateway_response;
					}

					return new WP_Error( 'onepipe_error', $gateway_response );
		...
		return new WP_Error( 'onepipe_error', 'This subscription can\'t be renewed automatically. The customer will have to login to his account to renew his subscription' );
